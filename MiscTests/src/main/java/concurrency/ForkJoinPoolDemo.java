 ...
 * workers who are busy.
 * 
 * Fork Join Framework Fork Join Framework It�s based on the work of Doug Lea, a
 * thought leader on Java concurrency. Fork/Join deals with the threading
 * hassles; you just indicate to the framework which portions of the work can be
 * broken apart and handled recursively. It employs pseudocode (as taken from
 * Doug Lea�s paper on the subject):
 * 
 * Result solve(Problem problem) { if (problem is small) directly solve problem
 * else { split problem into independent parts fork new subtasks to solve each
 * part join all subtasks compose result from subresults } }
 */
/*
 ...
 * 
 * If you use a no-argument constructor, by default, it creates a pool of size
 * that equals the number of available processors obtained using above
 * technique.
 */
