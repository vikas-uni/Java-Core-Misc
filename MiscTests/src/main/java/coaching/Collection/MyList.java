...
class Student {
	...
	public boolean equals(Object s) {
		if (s instanceof Student) {
			...
			return (stud.rollno == this.rollno) && (stud.name == this.name);
		}
		return false;
...
class MyList {
	...
	public static void main(String s[]) {
		...
		while (itr.hasNext()) { // checks if iteration has more elements& returns true if found
			Student st = (Student) itr.next(); // next() checks next elements of type Object(a ref var to Student here)
												// and downcast it into specified type
			st.exam((int) (Math.random() * 101)); // exam() in Student called for each Student object to store marks for
													// each Student
